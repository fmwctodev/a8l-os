  ...
  try {
    ...
    if (!supabaseUrl || !serviceRoleKey || !googleClientId) {
      return new Response(
        JSON.stringify({
          error:
            "Gmail integration is not configured. Please set GOOGLE_CLIENT_ID in Edge Function secrets.",
        }),
        {
          status: 500,
    ...
    let appRedirectUri = `${req.headers.get("Origin") || ""}/settings/profile?tab=connected-accounts`;
    if (req.method === "POST") {
      try {
    ...
    const oauthRedirectUri = `${supabaseUrl}/functions/v1/gmail-oauth-callback`;

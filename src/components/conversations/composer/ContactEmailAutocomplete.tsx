...
export function ContactEmailAutocomplete({
  ...
  return (
    <div className="flex items-start gap-2 px-4 py-1.5" ref={containerRef}>
      <span className="text-sm text-slate-400 mt-1 shrink-0">{label}:</span>
      <div className="flex-1 flex flex-wrap items-center gap-1 min-w-0 relative">
        {recipients.map((r, i) => (
          ...
          >
            {r.name || r.email}
            <button onClick={() => removeRecipient(i)} className="text-slate-400 hover:text-white">
              <X size={12} />
            </button>
          </span>
        ))}
        <div className="relative flex-1 min-w-[120px]">
          <input
            ...
            className="w-full text-sm bg-transparent border-none outline-none text-white placeholder-slate-500 py-0.5"
          />
          {showDropdown && (results.length > 0 || searching) && (
            <div className="absolute top-full left-0 mt-1 w-72 bg-slate-800 border border-slate-600 rounded-lg shadow-lg z-40 max-h-48 overflow-y-auto">
              {searching ? (
                ...
                results.map((contact) => (
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-slate-700 text-sm"
                  >
                    <div className="w-6 h-6 rounded-full bg-gradient-to-br from-cyan-500 to-teal-600 text-white flex items-center justify-center text-xs font-medium shrink-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-slate-500 text-xs truncate">{contact.email}</div>
                    </div>
                  </button>
                ))
              )}

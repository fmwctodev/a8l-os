...
export function MessageBubble({ message }: MessageBubbleProps) {
  ...
  return (
    <div className={`flex ${isOutbound ? 'justify-end' : 'justify-start'}`}>
      ...
      >
        {message.subject && (
          <div className={`text-xs font-medium mb-1 ${isOutbound ? 'text-blue-100' : 'text-gray-500'}`}>
        ...
        <div className={`flex items-center justify-end gap-2 mt-1.5 ${isOutbound ? 'text-blue-100' : 'text-gray-400'}`}>
...
function ChannelIndicator({ channel, isOutbound }: { channel: MessageChannel; isOutbound: boolean }) {
  const iconClass = `w-3 h-3 ${isOutbound ? 'text-blue-200' : 'text-gray-400'}`;
...
function StatusIndicator({ status }: { status: MessageStatus }) {
  switch (status) {
    ...
    case 'read':
      return <Eye size={12} className="text-blue-200" />;
    case 'failed':

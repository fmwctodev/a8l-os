...
export default function OpportunityFilesTab({ opportunity, onUpdate }: OpportunityFilesTabProps) {
  const { user } = useAuth();
  ...
  const loadAttachments = async () => {
    setLoading(true);
    try {
      const data = await getAttachments('opportunities', opportunity.id);
      setAttachments(data);
    } catch (err) {
      ...
      {attachments.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-gray-500 border-2 border-dashed border-gray-200 rounded-lg">
          ...
          <p className="text-sm text-gray-400 mb-4">
            Attach files from Google Drive to keep them organized with this opportunity
          </p>
          {canManage && (
            <button
              onClick={() => setShowAttachModal(true)}

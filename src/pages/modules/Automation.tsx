...
import { Search, Plus, Filter, Loader2, AlertCircle, Workflow as WorkflowIcon, MoreVertical, Play, Copy, Archive, Users, Zap, Clock, ChevronDown, LayoutGrid as Layout, CheckSquare, AlertTriangle, Activity, FolderOpen, Folder } from 'lucide-react';
      ...
      <div className="bg-slate-900 rounded-xl border border-slate-800">
              ...
              <div className="relative">
                ...
                {showFolderDropdown && (
                  <>
                    ...
                    />
                    <div className="absolute right-0 mt-2 w-56 max-h-72 overflow-y-auto bg-slate-800 border border-slate-700 rounded-lg shadow-xl z-20 py-1">
                      <button
                        onClick={() => {
                          setFolderFilter('all');
        ...
        <div className="divide-y divide-slate-800">
          {visibleWorkflows.map((workflow) => {
            const wfDef = (workflow.published_definition || workflow.draft_definition) as
              | { settings?: { folder?: string } }
              | null;
            const wfFolder = wfDef?.settings?.folder?.trim();
            return (
            <div
              key={workflow.id}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {wfFolder && (
                        <span
                          className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-slate-700/40 text-slate-300"
                          onClick={(e) => {
                            e.stopPropagation();
                            setFolderFilter(wfFolder);
                          }}
                          title={`Filter by folder: ${wfFolder}`}
                        >

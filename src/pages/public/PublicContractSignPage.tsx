...
import type { Contract, ContractSignatureRequest } from '../../types';
import {
  Scale,
  Loader2,
  AlertCircle,
...
export function PublicContractSignPage() {
  ...
  const [request, setRequest] = useState<ContractSignatureRequest | null>(null);
      ...
      const { data: contractData, error: contractError } = await supabase
        ...
        .select(`
          *,
          ...
          sections:contract_sections(id, title, content, section_type, sort_order, annotation)
        `)
  ...
  const handleSign = async () => {
    if (!request || !contract || !signatureData || !consentChecked) return;
    ...
    try {
      ...
      if (signatureData.imageDataUrl) {
        ...
        const path = `${contract.org_id}/${request.id}/signature.png`;
      ...
      const documentHash = contract.frozen_html_snapshot
        ? await hashToken(contract.frozen_html_snapshot)
        : 'no-snapshot';
  ...
  const handleDecline = async () => {
    ...
    try {
      ...
      await supabase
        .from('contracts')
        .update({ signature_status: 'declined', status: 'declined', declined_at: now })
        .eq('id', contract.id);
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-slate-900 rounded-2xl border border-slate-800 p-8 text-center">
          ...
          <p className="text-slate-400">This signing link is invalid or has been revoked. Please contact the sender for a new link.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-slate-900 rounded-2xl border border-slate-800 p-8 text-center">
          ...
          <p className="text-slate-400">This signing request has expired. Please contact the sender to request a new link.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-slate-900 rounded-2xl border border-slate-800 p-8 text-center">
          ...
          <p className="text-slate-400">This contract has already been signed. No further action is needed.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4" ref={topRef}>
        <div className="max-w-md w-full bg-slate-900 rounded-2xl border border-emerald-500/30 p-8 text-center">
          ...
          <p className="text-slate-400 mb-6">Your signature has been recorded. The sender has been notified.</p>
          <div className="p-4 bg-slate-800/60 rounded-lg border border-slate-700">
            <p className="text-sm text-slate-300">A signed copy of this contract will be available from the sender. You can safely close this page.</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4" ref={topRef}>
        <div className="max-w-md w-full bg-slate-900 rounded-2xl border border-slate-800 p-8 text-center">
          ...
          <h1 className="text-xl font-semibold text-white mb-2">Contract Declined</h1>
    ...
    <div className="min-h-screen bg-slate-950" ref={topRef}>
      <div className="max-w-3xl mx-auto px-4 py-8 sm:py-12">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <p className="text-sm text-slate-400">Contract for your review</p>
        ...
        <div className="flex items-center gap-2 mb-8">
          ...
          {request?.expires_at && (
            <span className="text-sm text-slate-400">Expires {formatDate(request.expires_at)}</span>
          )}
        </div>
        ...
        {(contract?.party_a_name || contract?.party_b_name) && (
          <div className="grid grid-cols-2 gap-4 mb-6 p-5 bg-slate-900 rounded-xl border border-slate-800">
            <div>
              <h3 className="text-xs font-semibold text-cyan-400 uppercase tracking-wider mb-2">Party A</h3>
              <p className="text-sm text-white font-medium">{contract.party_a_name || 'Not specified'}</p>
              {contract.party_a_email && <p className="text-xs text-slate-400 mt-1">{contract.party_a_email}</p>}
            </div>
            <div>
              <h3 className="text-xs font-semibold text-cyan-400 uppercase tracking-wider mb-2">Party B</h3>
          ...
          <div className="mb-6 p-5 bg-slate-900 rounded-xl border border-slate-800">
            <div className="flex items-center justify-between">
              ...
              <span className="text-2xl font-bold text-cyan-400 flex items-center gap-1">
                ...
                {formatCurrency(contract.total_value, contract.currency).replace('$', '')}
              </span>
            </div>
          </div>
        )}
        ...
        {sections.length > 0 && (
          <div className="space-y-3 mb-6">
            {sections.map((section) => (
              <div key={section.id} className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden">
                <button
                ...
                {expandedSections.has(section.id) && (
                  <div className="px-5 pb-5">
                    {section.annotation && (
                      <div className="mb-3 px-3 py-2 bg-slate-800/50 border-l-2 border-cyan-500/50 rounded-r-md">
        ...
        {pageState === 'decline_form' ? (
          <div className="bg-slate-900 rounded-xl border border-red-500/30 p-6 mb-6">
            <h3 className="text-lg font-semibold text-white mb-4">Decline This Contract</h3>
            <p className="text-sm text-slate-400 mb-4">Please let the sender know why you are declining. This is optional.</p>
            <textarea
              value={declineReason}
          ...
          <div className="bg-slate-900 rounded-xl border border-cyan-500/30 p-6 mb-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-lg font-semibold text-white">Sign This Contract</h3>
                <p className="text-sm text-slate-400">By signing, you agree to the terms outlined above</p>
              </div>
            </div>
            ...
            <label className="flex items-start gap-3 mb-6 cursor-pointer">
              ...
              <span className="text-sm text-slate-300">
                I agree to electronically sign this contract. I understand this constitutes a
